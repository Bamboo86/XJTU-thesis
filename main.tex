...
\title{面向长链条推理的几何问题基准构建与求解方法研究}{A Benchmark and Solving Method for Long-Chain Geometric Reasoning}
...
\author{朱佳印}{Jiayin Zhu}
% \author{***}{***}

% 指导老师姓名（注意：同作者姓名）
% Name of supervisor, It have the same requirments as the author
\advisor{张玲玲}{副教授}{Lingling Zhang}{Associate Prof.}
% \advisor{***}{***}{***}{***}
...
\addcommitteemember{西安交通大学,伍卫国,教授}
\addcommitteemember{航空631所,王世奎,研究员}
\addcommitteemember{西安交通大学,王鸽,副教授}
\addcommitteemember{西安交通大学,王志文,副教授}
\addcommitteemember{西安交通大学,钱屹,副教授}
% 答辩时间（手动指定）
% Defence date, input manually
\defensedate{2026}{05}{19}

% 答辩地点（涉密论文请手动设置为「西安交通大学」）
% Defence location, default value is 「西安交通大学」
\defenseloc{西安交通大学创新港校区4-6220}
...
% \usepackage{}
% \usepackage{amsmath} % 数学包必须在前面
% \usepackage{tabularx}
% \newcolumntype{Y}{>{\centering\arraybackslash}X} % 居中自适应列
\usepackage{array}      % 用于处理复杂的表格列格式
\usepackage{multirow}   % 用于支持 \multirow 命令
\usepackage[algo2e, linesnumbered, ruled, vlined]{algorithm2e}
% 将算法计数器 algocf 与 chapter 绑定，并修改显示格式
